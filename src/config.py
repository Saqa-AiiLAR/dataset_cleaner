...
class Config:
    ...
    pattern_matching_sensitivity: float = (
        0.8  # Threshold for morphological pattern matching (0.0-1.0)
    )
    ...
    def validate(self) -> None:
        ...
        if self.progress_interval_pages < 1:
            raise ValidationError(
                f"progress_interval_pages must be >= 1, got {self.progress_interval_pages}"
            )

        if self.progress_interval_words < 1:
            raise ValidationError(
                f"progress_interval_words must be >= 1, got {self.progress_interval_words}"
            )
        ...
        if (
            self.word_healer_exceptions_file is not None
